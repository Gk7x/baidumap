...
public class HouseSearchService {
     ...
     * @return
     */
    public List<HouseResultVo> search(Double maxLongitude, Double minLongitude, Double maxLatitude, Double minLatitude, Double zoom){
            //收集聚合查询条件
        List<AggregationOperation> operationList = new ArrayList<>();

        //在可视范围内搜索     两个点-> 最大经度+纬度   |  最小经度+纬度
        Box box = new Box(new double[]{maxLongitude,maxLatitude},new double[]{minLongitude,minLatitude});
        //在 location 表中 查询  在↑ 范围的数据
        ...
        switch(type){
            ...
            case 2:{
                //查询商圈数据
                for (HouseResultVo houseResultVo:houseResultVoList){
                    BusinessCircle businessCircle = this.queryBusinessCircletByCode(Convert.toInt(houseResultVo.getCode()));
...
根据code查询商圈数据
 */
    private BusinessCircle queryBusinessCircletByCode(Integer code){
        Query query = Query.query(Criteria.where("code").is(code));
...
根据code查询小区数据
*/
    private Community queryCommunityById(Object id){
